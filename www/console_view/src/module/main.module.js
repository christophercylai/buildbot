...
class Console {
    constructor($scope, $q, $window, dataService, bbSettingsService, resultsService,
        ...
        this.defaultBranch = settings.defaultBranch.value;
    ...
    _onChange() {
        ...
        if (this.builders.length == 0) { return; }
        ...
        for (build of Array.from(this.builds)) {
            try { this.matchBuildWithChange(build); } catch(err) {}
        }
    ...
    sortBuildersByTags(all_builders) {
        ...
        let branches = new URL(url).hash.split("?")[1];
        if ((branches == null)) {
            branches = this.defaultBranch
        ...
        for (let builder of Array.from(all_builders)) {
            for (let branch of Array.from(branches)) {
                if (builder.hasBuild && builder.name.includes(branch)) {
    ...
    matchBuildWithChange(build) {
        ...
        if ((change == null) && ((build.properties != null ? build.properties.got_revision : undefined) != null)) {
            if ((build.properties.author != null)) {
                author = build.properties.author[0].split('@')[0];
            }
            if ((build.properties.comment != null)) {
            ...
            if (typeof(rev) === "string") {
                ...
                if ((change == null)) {
                    change = this.makeFakeChange("", rev, author, comment, build.started_at);
                }
            } else {
                let codebase;
                for (codebase in rev) {
                ...
                if ((change == null)) {
                    ...
                    change = this.makeFakeChange(codebase, author, comment, revision, build.started_at);
                }
            }
        }
        ...
        if ((change == null)) {
            ...
            change = this.makeFakeChange("unknown codebase", revision, author, comment, build.started_at);
        }
